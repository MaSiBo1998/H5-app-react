...
export default function LoanDetailPopup({
  ...
  const loanList = useMemo(() => {
    ...
    const beirut = productData.beirut || 0 // Service Fee Total
    const gaucho = productData.gaucho || 0 // Tax Total
    ...
    for (let i = 1; i <= periods; i++) {
      ...
      let daysToAdd = daysPerPeriod * i
      // 保持与 LoanUnconfirmed.tsx 一致的逻辑：单期产品少算一天
      if (periods === 1) {
        daysToAdd = daysToAdd - 1
      }
      
      currentRepayDate.setDate(startDate.getDate() + daysToAdd)
